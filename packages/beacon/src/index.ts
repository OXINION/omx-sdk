...
export class BeaconManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize beacon manager: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async startScanning(options?: ScanOptions): Promise<void> {
    ...
    if (!this.startTime) {
      throw new Error(
        'Beacon manager not initialized. Call initialize() first.'
      );
    }
   ...
   */
  estimateDistance(rssi: number, txPower = -59): number {
    if (rssi === 0) {
      return -1;
    }
...
export function createBeaconManager(config: BeaconConfig): BeaconManager {
