...
async function testOMXInitialization() {
    ...
    try {
        ...
        console.log('📡 POST https://blhilidnsybhfdmwqsrx.supabase.co/functions/v1/create-jwt-token');
    ...
    try {
        ...
        if (isBrowser) {
            await geotrigger.startMonitoring((event) => {
                console.log(`🎯 Geofence Event: ${event.type} region ${event.regionId}`);
      ...
      `,
        ...
        const subscription = await webhook.createSubscription('https://your-app.com/webhook/handler', ['user.created', 'user.updated', 'order.completed'], 'your-webhook-secret');
        ...
        if (isBrowser) {
            try {
                ...
                beacon.addEventListener('range', (event) => {
                    console.log(`📡 Ranging in ${event.region.name}: ${event.beacons?.length} beacons`);
                });
                // Start scanning
                ...
                setTimeout(() => {
                    ...
                    console.log(`📡 Discovered ${discoveredBeacons.length} beacons:`, discoveredBeacons);
                }, 5000);
            }
            catch (error) {
        ...
        if (isBrowser) {
            try {
                ...
                if (pushSubscription.success) {
                    ...
                    console.log('❌ Push notification subscription failed:', pushSubscription.error);
                }
            }
            catch (error) {
